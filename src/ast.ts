...
import { parse as _parse, parseExpression, ParserOptions, ParserPlugin } from "@babel/parser";
...
import { CSS_VARS_HELPER, genCssVarsCode, genNormalScriptCssVarsCode } from "./cssVars";
...
export function compileScript(sfc: SFCDescriptor, options: SFCScriptCompileOptions): SFCScriptBlock {
  ...
  if (!isTS || scriptLang === "tsx" || scriptSetupLang === "tsx") {
    ...
    if (options.babelParserPlugins) options.babelParserPlugins = options.babelParserPlugins.filter((n) => n !== "jsx");
  }
    ...
    try {
      ...
      if (enableReactivityTransform && shouldTransform(content)) {
        ...
        if (importedHelpers.length) {
          s.prepend(`import { ${importedHelpers.map((h) => `${h} as _${h}`).join(", ")} } from 'vue'\n`);
        }
  ...
  let emitsTypeDecl: TSFunctionType | TSTypeLiteral | TSInterfaceBody | undefined;
  ...
  function parse(input: string, options: ParserOptions, offset: number): Program {
    try {
  ...
  function error(msg: string, node: Node, end: number = node.end! + startOffset): never {
    throw new Error(
  ...
  function processDefineProps(node: Node, declId?: LVal): boolean {
    ...
    if (node.typeParameters) {
      ...
      propsTypeDecl = resolveQualifiedType(propsTypeDeclRaw, (node) => node.type === "TSTypeLiteral") as
        | TSTypeLiteral
        | TSInterfaceBody
        | undefined;
  ...
  function processDefineEmits(node: Node, declId?: LVal): boolean {
    ...
    if (declId) {
      emitIdentifier =
        declId.type === "Identifier" ? declId.name : scriptSetup!.content.slice(declId.start!, declId.end!);
    }
  ...
  function resolveExtendsType(node: Node, qualifier: (node: Node) => boolean, cache: Array<Node> = []): Array<Node> {
  ...
  function isQualifiedType(node: Node, qualifier: (node: Node) => boolean, refName: String): Node | undefined {
  ...
  function resolveQualifiedType(node: Node, qualifier: (node: Node) => boolean) {
    if (qualifier(node)) {
      return node;
    }
    if (node.type === "TSTypeReference" && node.typeName.type === "Identifier") {
      ...
      const body = getAstBody();
      for (const node of body) {
        let qualified = isQualifiedType(node, qualifier, refName) as TSInterfaceBody;
        if (qualified) {
   ...
   * )
   */
  function processAwait(node: AwaitExpression, needSemi: boolean, isStatement: boolean) {
    const argumentStart =
    ...
    s.overwrite(
      ...
      `${needSemi ? `;` : ``}(\n  ([__temp,__restore] = ${helper(`withAsyncContext`)}(${
  ...
  function hasStaticWithDefaults() {
    return (
      ...
      propsRuntimeDefaults.properties.every(
        (node) =>
          (node.type === "ObjectProperty" && (!node.computed || node.key.type.endsWith("Literal"))) ||
        ...
        if (destructured) {
          ...
          if (prop) {
            if (prop.type === "ObjectProperty") {
              ...
              }default() ${scriptSetupSource.slice(prop.body.start!, prop.body.end!)}`;
            }
          }
        }
        ...
        if (
          ...
          if (
            propsRuntimeDefaults!.properties.some((p) => {
              ...
              return resolveObjectKey(p.key, p.computed) === (m.key as Identifier).name;
            })
          ) {
            res +=
              m.key.name +
  ...
  if (propsDestructureDecl) {
    if (propsDestructureRestId) {
      bindingMetadata[propsDestructureRestId] = BindingTypes.SETUP_REACTIVE_CONST;
    }
  ...
  if (propsDestructureRestId) {
    s.prependLeft(
      startOffset,
      `\nconst ${propsDestructureRestId} = ${helper(`createPropsRestProxy`)}(__props, ${JSON.stringify(
...
function walkDeclaration(
  ...
  if (node.type === "VariableDeclaration") {
    ...
    for (const { id, init } of node.declarations) {
      const isDefineCall = !!(
        isConst && isCallOf(init, (c) => c === DEFINE_PROPS || c === DEFINE_EMITS || c === WITH_DEFAULTS)
      );
...
function inferRuntimeType(node: TSType, declaredTypes: Record<string, string[]>): string[] {
  switch (node.type) {
    ...
    case "TSUnionType":
      return [...new Set([].concat(...(node.types.map((t) => inferRuntimeType(t, declaredTypes)) as any)))];
...
function extractRuntimeEmits(node: TSFunctionType | TSTypeLiteral | TSInterfaceBody, emits: Set<string>) {
...
function extractEventNames(eventName: Identifier | RestElement, emits: Set<string>) {
  if (
        ...
        if (node.type === NodeTypes.ELEMENT) {
          ...
          for (let i = 0; i < node.props.length; i++) {
            ...
            if (prop.type === NodeTypes.DIRECTIVE) {
              ...
              if (prop.exp) {
                code += `,${processExp((prop.exp as SimpleExpressionNode).content, prop.name)}`;
              }
            }
          }
 ...
 */
export function hmrShouldReload(prevImports: Record<string, ImportBinding>, next: SFCDescriptor): boolean {
